...
class SimpleMLP(nn.Module):
    def __init__(self, weight_tensor, num_labels):
        super().__init__()
        ...
        filters_num = 30
        window_size = 3
        self.linear1 = nn.Linear((filters_num + 50) * 5, hidden_dim)
        self.dropout = nn.Dropout(0.5)
        self.linear2 = nn.Linear(hidden_dim, num_labels)
        self.conv = nn.Conv1d(in_channels=30, out_channels=filters_num, kernel_size=window_size, stride=1, padding=1)
...
def preprocess(path_train: str, path_dev: str, path_test: str):
    with open(path_train, 'r') as file:
        TRAIN = extract_windows(file.readlines(), False)
    with open(path_dev, 'r') as file:
        VALIDATION = extract_windows(file.readlines(), False)
    with open(path_test, 'r') as file:
        TEST = extract_windows(file.readlines(), True)
...
def train_model(task_name):
    ...
    model = SimpleMLP(weight_tensor, len(label_vocab))
    model.eval()
    ...
    # predict_test(model, test_dataloader, label_vocab, task_name)
...
def rebuild_test_results(task_name: str):
    train, _, test = preprocess(f"{task_name}/train", f"{task_name}/dev", f"{task_name}/test")
    vocab, _ = create_vocab_from_all(train, task_name)
    label_vocab = build_label_vocab(train)
    test_dataset = InitDataset(test, vocab, label_vocab, task_name=task_name, test=True)
    test_dataloader = DataLoader(test_dataset, 1, shuffle=False)
    model = load_model(f"checkpoints/{task_name}/final")
    predict_test(model, test_dataloader, label_vocab, task_name)


def predict_test(model, test_dataloader, label_vocab, task_name: str):
    with open(f"{task_name}/test") as test_data_file:
        test_data = iter(test_data_file.readlines())
    res_file = open(f'test1.{task_name}', 'w')
    for i, (sample, _) in enumerate(test_dataloader):
        output = model(sample)
        predictions = torch.argmax(output, dim=1)
        original_word = next(test_data)
        while original_word == "\n" or original_word == "-DOCSTART-\n":
            res_file.write(original_word)
            original_word = next(test_data)
        nx = original_word.replace("\n", "")
        res_file.write(f"{nx} {label_vocab.lookup_token(predictions.item())}\n")
    res_file.close()
