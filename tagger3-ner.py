from tagger3 import create_vocab_from_all, simple_mlp_pretrain_words
import torch
...
def train_ner_pretrained(case, path_train, path_dev, path_test):
    epochs = 50
    embedding_dim = 50
    hidden_dim = 50
    device_ids = [0]
    ...
    if case == "continue":
        model.load_state_dict(torch.load("models/ass1/ner/checkpoint-3"))
        model.eval()
    model.to(device)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if epoch%10 == 0:
            torch.save(model, f"models/ass1/ner/3-checkpoint-{epoch//10}")
            create_graphs(f"3-ner_plot-loss-{epoch//10}",loss_acc_all, epoch,"loss")
            create_graphs(f"3-ner_plot-acc-{epoch//10}",dev_acc_all, epoch,"acc")
    ...
    torch.save(model, "models/ass1/ner/3-checkpoint-1")

def predict_test_ner_pretrain(path_weights, test_dataset, embedding_dim, hidden_dim, label_vocab, vocab):
...
if __name__ == "__main__":
    ...
    label_vocab = get_vocab(train, input=False)
    predict_test_ner_pretrain("models/ass1/ner/3-checkpoint-2", test, 50, 50, label_vocab, vocab)
    # train_ner_pretrained("dev", "ner/train", "ner/dev", "ner/test" )
